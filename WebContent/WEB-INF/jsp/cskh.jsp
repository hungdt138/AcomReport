		...
		<aside class="right-side">
			...
			<section class="content">
							...
							<table border="0" cellpadding="0" cellspacing="0">
								<tr>
									<c:if test="${role == 'admin'}">
										...
										<td>Đối tác:</td>
										<td style="padding-left: 10px;"><select name="mId"
											class="form-control">
										...
										<td style="padding-left: 15px;">Nhà mạng</td>
								...
								<tr>
									...
									<td style="padding-left: 10px;"><input
										class="form-control date-picker" id="datepicker" type="text"
										name="f" value="${fromDate}" /></td>
									<td style="padding-left: 15px;">Đến ngày</td>
									<td style="padding-left: 10px;"><input
										class="form-control date-picker" id="datepicker1" type="text "
										name="t" value="${toDate}" /></td>
								</tr>
								<tr>
									<td>ISDN</td>
									<td style="padding-left: 10px;"><input
										class="form-control" name="isdn" value="${isdn}"></td>
									<td style="padding-left: 15px;">Telco ServiceID</td>
									<td style="padding-left: 10px;"><input
										class="form-control" name="tsId" value="${telcoServiceId}">
									</td>
									<td style="padding-left: 15px;">ProductID</td>
									<td style="padding-left: 10px;"><select name="pId"
										class="form-control">
											<option value="0">Tất cả</option>
											<c:forEach items="${lstProductCat}" var="e" step="1"
												begin="0">
									...
									<td style="padding-left: 10px;"><select
										class="form-control" name="action">
											...
											<c:if test="${action == 'register'}">
												<option value="0">Tất cả</option>
												<option value="register" selected="selected">Đăng
													ký</option>
												<option value="unregister">Hủy dv</option>
												<option value="otp-req">Request OTP</option>
												<option value="sub-req">Request Sub</option>
											...
											<c:if test="${action == 'unregister'}">
												<option value="0">Tất cả</option>
												<option value="register">Đăng ký</option>
												<option value="unregister" selected="selected">Hủy
													dv</option>
												<option value="otp-req">Request OTP</option>
												<option value="sub-req">Request Sub</option>
												<option value="send-mt">Gửi SMS</option>
											...
											<c:if test="${action =='otp-req'}">
												...
												<option value="register">Đăng ký</option>
												<option value="unregister">Hủy dv</option>
												<option value="otp-req" selected="selected">Request
													OTP</option>
												<option value="sub-req">Request Sub</option>
												<option value="send-mt">Gửi SMS</option>
												<option value="confirm">Trả lời câu hỏi</option>
											...
											<c:if test="${action == 'sub-req'}">
												...
												<option value="unregister">Hủy dv</option>
												<option value="otp-req">Request OTP</option>
												<option value="sub-req" selected="selected">Request
													Sub</option>
												<option value="send-mt">Gửi SMS</option>
												<option value="confirm">Trả lời câu hỏi</option>
												<option value="smsdaily">Gửi tin hằng ngày</option>
											...
											<c:if test="${action == 'confirm'}">
												...
												<option value="sub-req">Request Sub</option>
												<option value="send-mt">Gửi SMS</option>
												<option value="confirm" selected="selected">Trả lời
													câu hỏi</option>
												<option value="smsdaily">Gửi tin hằng ngày</option>
											</c:if>
											<c:if test="${action == 'smsdaily'}">
												...
												<option value="send-mt">Gửi SMS</option>
												<option value="confirm">Trả lời câu hỏi</option>
												<option value="smsdaily" selected="selected">Gửi
													tin hằng ngày</option>
											</c:if>
									</select></td>
								</tr>
				...
				<div class="filter-container" style="margin-bottom: 10px;">
					<span style="font-size: 17px;">Có <b>${totalRc}</b> bản ghi.
					</span>
									...
									<c:forEach items="${lst}" var="l">
										<tr style="text-align: center;">
											...
											<td><c:if test="${l.orderType == 'otp-req'}">
													...
													test="${l.orderType == 'sub-req'}">
                                                        Request Sub
                                                    </c:if> <c:if
													test="${l.orderType == 'unregister'}">
                                                        Hủy dv
                                                    </c:if> <c:if
													test="${l.orderType == 'register'}">
                                                        DK dv
                                                    </c:if> <c:if
													test="${l.orderType == 'send-mt'}">
                                                        Gửi sms
                                                    </c:if> <c:if
													test="${l.orderType == 'confirm'}">
                                                        Trả lời câu hỏi
                                                    </c:if></td>
											<td>${l.isdn}</td>
						...
						<div class="row">
							...
							<div class="text-right" style="padding-right: 20px;">
								<ul class="pagination">
									<c:if test="${page > 1}">
										<li><a
											href="?p=1&isdn=${isdn}&pId=${productId}&tsId=${telcoServiceId}&mId=${merchantId}&tId=${tId}&f=${fromDate}&t=${toDate}&r=${rowDisplay}&action=${action}">Trang
												đầu</a></li>
										<li><a
											href="?p=${page - 1}&isdn=${isdn}&pId=${productId}&tsId=${telcoServiceId}&mId=${merchantId}&tId=${tId}&f=${fromDate}&t=${toDate}&r=${rowDisplay}&action=${action}">&laquo;</a></li>
									</c:if>
									...
									<c:if test="${page < totalPage}">
										<li><a
											href="?p=${page + 1}&isdn=${isdn}&pId=${productId}&tsId=${telcoServiceId}&mId=${merchantId}&tId=${tId}&f=${fromDate}&t=${toDate}&r=${rowDisplay}&action=${action}">&raquo;</a></li>
										<li><a
											href="?p=${totalPage}&isdn=${isdn}&pId=${productId}&tsId=${telcoServiceId}&mId=${merchantId}&tId=${tId}&f=${fromDate}&t=${toDate}&r=${rowDisplay}&action=${action}">Trang
												cuối</a></li>
									</c:if>
									<c:if test="${page >= totalPage}">
